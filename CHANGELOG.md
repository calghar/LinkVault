...
- **A new note is created when nothing matches.** When no existing note covers a link, LinkVault names one from the link's content and files it there. A link is only left in the Inbox if no usable name can be produced.
...
- **New notes follow the structure of your existing ones**: title, tag line, a `[[Index]] →` backlink describing what the note collects, and a first section with an empty link table. The table header comes from your configured header marker rather than a fixed one. Previously an unmatched response fell through to the *first* file in the list, so links were silently misrouted.
